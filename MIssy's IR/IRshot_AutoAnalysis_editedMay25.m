...
Delta_Data_zoom = Data(zoom_y-200:zoom_y,zoom_x-100:zoom_x+100,f1) - Data(zoom_y-200:zoom_y,zoom_x-100:zoom_x+100,f0);
figure;
image(Delta_Data_zoom,'CDataMapping','scaled');
TC_circle = viscircles([x_c y_c],TC_radius);
centerline_horiz = imline(gca, [x_c-5 y_c-87; x_c+5 y_c-87]);
centerline_vert = imline(gca, [x_c y_c-82;x_c y_c-92]);
colormap jet
c=colorbar;
grid on
...
Data_zoom = Data(zoom_y-200:zoom_y,zoom_x-100:zoom_x+100,1:zoomframes);
...
for i=1:zoomframes
    meanlist(i)= mean2(DeltaTMatrix(y_c2-(87*2):y_c2,x_c2-87:x_c2+87,i));
    meanlist_step(i) = mean2(DeltaTMatrix_step(y_c2-(87*2):y_c2, x_c2-87:x_c2+87,i));
end
...
if framenumber-hel_frame>=3
    if abs(meanlist_step(framenumber))<0.3
        while abs(hel_frame-framenumber)>=2
                framenumber=framenumber-1;
        end
    else
...
if hel_frame-framenumber>=3
    if abs(meanlist_step(hel_frame))<0.3
        while abs(hel_frame-framenumber)>=2
                hel_frame=hel_frame-1;
        end
    else
...
ellipse=imellipse(gca,[111-r2 105-r1 diameter large_diameter]);
...
ellipse2=imellipse(gca,[100-87 200-(87*2) 87*2 87*2]);
...
centerline_horiz = imline(gca, [x_c2-5 y_c2-87; x_c2+5 y_c2-87]);
centerline_vert = imline(gca, [x_c2 y_c2-82;x_c2 y_c2-92]);
ellipse5=imellipse(gca,[column-3 row-3 6 6]);
ellipse6 = imellipse(gca,[center1-3 center2-3 6 6]);
rect_mean = imrect(gca,[x_c2-87 y_c2-(2*87) 87*2 87*2]);
...
%     ech_end_time = input('when did the ech turn off after t=+4 sec [sec]?  ');
    ...
    %rows] 
    %Approximate 'E1' from ResearchIR
    ellipseECH=imellipse(gca,[111-r2 105-r1 diameter large_diameter]);
    ...
    %Approximate 'E2' from ResearchIR
    ellipseECH2=imellipse(gca,[100-87 200-(87*2) 87*2 87*2]);
...
%     ich_end_time = input('when did the ich turn off after t=+4 sec [sec]?  ');
    ...
    %rows] 
    %Approximate 'E1' from ResearchIR
    ellipseICH=imellipse(gca,[111-r2 105-r1 diameter large_diameter]);
    ...
    %Approximate 'E2' from ResearchIR
    ellipseICH2=imellipse(gca,[100-87 200-(87*2) 87*2 87*2]);
...
%     
    ...
    %rows] 
    %Approximate 'E1' from ResearchIR
    ellipseECH_ICH=imellipse(gca,[111-r2 105-r1 diameter large_diameter]);
    ...
    %Approximate 'E2' from ResearchIR
    ellipseECH_ICH2=imellipse(gca,[100-87 200-(87*2) 87*2 87*2]);
...
save(filename,'Frame','shotnumber','zoomframes','TG_frame','finalframenumber','TC_radius','px_per_cm','emax','helicon_start','center_plate','TG_templist','hel_templist','TG_templist_step','hel_templist_step','DeltaTMatrix_step','meanlist_step','meanlist','area_TG','avg_area','x_c2','y_c2','diameter', 'plasmaframe');
