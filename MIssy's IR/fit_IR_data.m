...
if plotit
    ...
    if manual_select
        ...
        rows]
        TC_diameter = 18/px_per_cm;
        TC_radius = (TC_diameter/2)/px_per_cm;
        TC_circle = viscircles([x_c y_c],TC_radius);
        centerline_horiz = imline(gca, [x_c-(5/px_per_cm) y_c+(87/px_per_cm); x_c+(5/px_per_cm) y_c+(87/px_per_cm)]);
        centerline_vert = imline(gca, [x_c y_c+(82/px_per_cm);x_c y_c+(92/px_per_cm)]);
        center_plate=[x_c,y_c+(87/px_per_cm)];
        x0_guess=x_c;
        y0_guess=y_c+(87/px_per_cm);
...
pick_trace = input('Do you want to automatically trace through the hottest spot or through the hottest edge? [Hot spot=1, hot edge=2] '); %Pick trace option - if hot spot is in center, impt edge effects could be missed
...
if pick_trace == 2
    angle = atan2(y0_guess2,x0_guess2);
    if plotit
        plot(x0_guess2, y0_guess2,'ko')
    end
end
