...
Raw_data = readSPE('Z:\McPherson\2016_05_11\D2_1669_20um_19.SPE');...
    %USER Specifiy Location
length = size(Raw_data);
Raw_data_bg = readSPE('Z:\McPherson\calibration\absolute_intensity_calib\absolute_1669_20um_bg_1.SPE');...
    %USER Specify Location

P_o = 206; %USER put peaklocation here!!! peak location can change it is not the same each time
lambda_o = 667.8; %USER put wavelength here, what you tunned the Mcpherson to in nanometers!!!
...
ax = gca;
ax.FontSize = 20;
title('Counts vs Corrected Pixels','FontSize',20);
...
ax = gca;
ax.FontSize = 20;
title('Intensity vs wavelength (nm)','FontSize',20);
xlabel('Wavelength (nm)','FontSize',20);
ylabel('Intensity (mW/cm2-micron)','FontSize',20);
%}
