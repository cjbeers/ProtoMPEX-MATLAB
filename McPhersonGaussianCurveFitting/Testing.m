...
Raw_data = readSPE('C:\Users\cxe\Documents\McPherson\SplineExampleData\D2_ECH_1219_20um_9024.SPE');...
    %USER Specifiy Location
length = size(Raw_data);
Raw_data_bg = readSPE('C:\Users\cxe\Documents\McPherson\SplineExampleData\absolute_1469_20um_bg_1.SPE');...
...
P_o = 217; %USER put peaklocation here!!! peak location can change it is not the same each time
...
[pks,locs] = findpeaks(flip(cor), flip(lambdaplot), 'MinPeakHeight',1500,'MinPeakDistance',10)
correctedtable = zeros(2,512);
correctedtable(1,:)=lambdaplot;
correctedtable(2,:)=cor;
...
%findpeaks1(x,y,SlopeThreshold,AmpThreshold,smoothwidth,peakgroup,smoothtype)
