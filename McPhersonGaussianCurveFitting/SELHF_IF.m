...
%************************************
%Wavelength bounds for data selection
%************************************
WD=[4675 4715];                                     
...
if PLOT_FIT==1
    ...
    for jj=1:ROI
        ...
        plot(XFIT{jj}(1:NFP),IFIT{jj}(1:NFP),'-r','LineWidth',7)
        if PLOT_GAU==1
            for ii=1:NG
                plot(XFIT_GAU{jj,ii}(1:NFP),IFIT_GAU{jj,ii}(1:NFP),'-b','LineWidth',3)
            end
        end
        hold off
        xlabel(['Wavelength (' char(197) ')'],'FontSize',40)
