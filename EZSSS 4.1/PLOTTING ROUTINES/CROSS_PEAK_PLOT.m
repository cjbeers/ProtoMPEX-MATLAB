function CROSS_PEAK_PLOT(SPECTRA,FORMAT,OPT)
...
%***************************************
X_ST=DISC.FULL.PEAK.X;
LL_ST=DISC.FULL.PEAK.STATE.LL;
UL_ST=DISC.FULL.PEAK.STATE.UL;
QSAN_ST=DISC.FULL.PEAK.STATE.QSAN;
IND_ST=DISC.FULL.PEAK.STATE.IND;
...
if SUM_LOG==1
    ...
    I_D=DISC.SUM.PEAK.I;
    X_D=DISC.SUM.PEAK.X;
    NT_D=DISC.SUM.PEAK.NT;
    ND_D=DISC.SUM.PEAK.ND;
    ...
    %Assign the crossover spectra
    %****************************
    I_D=DISC.FULL.PEAK.I;
    X_D=DISC.FULL.PEAK.X;
    NT_D=DISC.FULL.PEAK.NT;
    ND_D=[];
    ...
    if IND_LOG==1
        ...
        for ii=1:NT_D
            ST_LABEL{ii}={['LL ' num2str(LL_ST(ii))],['UL ' num2str(UL_ST(ii,1)) ',' num2str(UL_ST(ii,2))],['QS ' num2str(QSAN_ST(ii))]};
        end
...
if CONT_LOG==1
    I_C=SPECTRA.CONT.PEAK.I;
    X_C=SPECTRA.CONT.PEAK.X;
end

%****************************
%Return if no crossover peaks
%****************************
if NT_D==0
    return
end
...
for ii=1:NG
    ...
    if PLOT_DISC==1 && PLOT_LL==1
        ...
        title(['Discrete Crossover Peak Spectra' SIGN_PK],'FontSize',38)
        set(gca,'XTick',XTICK)
        set(gca,'XTickLabel',[])
        axis(AM.D)
        grid on   
    ...
    if PLOT_DISC==1 && PLOT_UL==1
        ...
        subplot(6,1,4:6)
        hold on
        plot(X_ST*1e10,UL_ST(:,1),'o','MarkerEdgeColor','k','MarkerFaceColor','k')
        plot(X_ST*1e10,UL_ST(:,2),'o','MarkerEdgeColor','k','MarkerFaceColor','k')
        hold off
        xlabel(XNAME,'FontSize',38)
    ...
    if PLOT_DISC==1
        ...
        legend([PH1 PH2],{'Atomic',['Crossover Peak' SIGN_PK]})
        xlabel(XNAME,'FontSize',38)
        ylabel(YNAME.DISC,'FontSize',38)
    ...
    if PLOT_CONT==1 && CONT_LOG==1
        ...
        title(['Continuous Crossover Peak Spectra' SIGN_PK],'FontSize',38,'Color','k','FontWeight','Bold')
