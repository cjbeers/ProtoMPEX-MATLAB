function dALPHA=DIFF_ABSORPTION_COEFF(VX,VY,NU,I_PR,I_PU,PARA,UNIV)
...
if DIMENSION==1
    dNG=dNG*(2*pi*q*KT/m)^.5;
    dNE=dNE*(2*pi*q*KT/m)^.5;
end
...
%****************************
if strcmpi(MODE,'PUMP')==1
    dALPHA=SIG_PU.*(dNG*G-dNE); 
elseif strcmpi(MODE,'PROBE')==1 || strcmpi(MODE,'BOTH')==1
    dALPHA=SIG_PR.*(dNG*G-dNE);  
end
