function CHI=Fun_1a(PARA,DATA,OPT,PSF)
...
for uu=1:NSPF
    ...
    if (PSN(2)<PSNM(2))
        ...
        POL(1:PSNM(1))=PSF(PSN(1)+1:sum(PSN(1:2)));
    end
    ...
    if (PSN(4)<PSNM(4))
        ...
        for jj=1:PSNM(4)
            if (PSL(sum(PSNM(1:3))+jj)==0)
                kk=kk+1;
                EFP_X(jj)=PSF(sum(PSN(1:3))+kk);
            else
                EFP_X(jj)=PSV(sum(PSNM(1:3))+jj);
            end
        end
    else
        EFP_X(1:PSNM(4))=PSF(sum(PSN(1:3))+1:sum(PSN(1:4)));
    end
    ...
    if (PSN(5)<PSNM(5))
        ...
        for jj=1:PSNM(5)
            if (PSL(sum(PSNM(1:4))+jj)==0)
                kk=kk+1;
                EFP_Y(jj)=PSF(sum(PSN(1:4))+kk);
            else
                EFP_Y(jj)=PSV(sum(PSNM(1:4))+jj);
            end
        end
    else
        EFP_Y(1:PSNM(5))=PSF(sum(PSN(1:4))+1:sum(PSN(1:5)));
    end
    ...
    if (PSN(6)<PSNM(6))
        ...
        for jj=1:PSNM(6)
            if (PSL(sum(PSNM(1:5))+jj)==0)
                kk=kk+1;
                EFP_Z(jj)=PSF(sum(PSN(1:5))+kk);
            else
                EFP_Z(jj)=PSV(sum(PSNM(1:5))+jj);
            end
        end
    else
        EFP_Z(1:PSNM(6))=PSF(sum(PSN(1:5))+1:sum(PSN(1:6)));
    end

    %**************************************
    %Defining neutral temperature of groups
    %**************************************
    if (PSN(7)<PSNM(7))
        ll=0;
        for kk=1:PSNM(7)
            if (PSL(sum(PSNM(1:6))+kk)==0)
                ll=ll+1;
                kT_DOP(kk)=PSF(sum(PSN(1:6))+ll);
            else
                kT_DOP(kk)=PSV(sum(PSNM(1:6))+kk);
            end
        end
    else
        kT_DOP(1:PSNM(7))=PSF(sum(PSN(1:6))+1:sum(PSN(1:7)));
    end

    %********************************
    %Defining Doppler shift of groups
    %********************************
    if (PSN(8)<PSNM(8))
        ll=0;
        for kk=1:PSNM(8)
            if (PSL(sum(PSNM(1:7))+kk)==0)
                ll=ll+1;
                X_DOP(kk)=PSF(sum(PSN(1:7))+ll);
            else
                X_DOP(kk)=PSV(sum(PSNM(1:7))+kk);
            end
        end
    else
        X_DOP(1:PSNM(8))=PSF(sum(PSN(1:7))+1:sum(PSN(1:8)));
    end

    %***************************************
    %Defining relative intensities of groups
    %***************************************
    if (PSN(9)<PSNM(9))
        ll=0;
        for kk=1:PSNM(9)
            if (PSL(sum(PSNM(1:8))+kk)==0)
                ll=ll+1;
                I_DOP(kk)=PSF(sum(PSN(1:8))+ll);
            else
                I_DOP(kk)=PSV(sum(PSNM(1:8))+kk);
            end
        end
    else
        I_DOP(1:PSNM(9))=PSF(sum(PSN(1:8))+1:sum(PSN(1:9)));
    end 
    ...
    if (PSN(10)<PSNM(10))
        ...
        for jj=1:PSNM(10)
            if (PSL(sum(PSNM(1:9))+jj)==0)
                kk=kk+1;
                GAM_LOR(jj)=PSF(sum(PSN(1:9))+kk);
            else
                GAM_LOR(jj)=PSV(sum(PSNM(1:9))+jj);
            end
        end
    elseif (PSNM(10)>0)
        GAM_LOR(1:PSNM(10))=PSF(sum(PSN(1:9))+1:sum(PSN(1:10)));
    end
    ...
    if (PSN(11)<PSNM(11))
        ...
        for jj=1:PSNM(11)
            if (PSL(sum(PSNM(1:10))+jj)==0)
                kk=kk+1;
                X_LOR(jj)=PSF(sum(PSN(1:10))+kk);
            else
                X_LOR(jj)=PSV(sum(PSNM(1:10))+jj);
            end
        end
    elseif (PSNM(11)>0)
        X_LOR(1:PSNM(11))=PSF(sum(PSN(1:10))+1:sum(PSN(1:11)));
    end
    ...
    if (PSN(12)<PSNM(12))
        ...
        for jj=1:PSNM(12)
            if (PSL(sum(PSNM(1:11))+jj)==0)
                kk=kk+1;
                I_LOR(jj)=PSF(sum(PSN(1:11))+kk);
            else
                I_LOR(jj)=PSV(sum(PSNM(1:11))+jj);
            end
        end
    elseif (PSNM(12)>0)
        I_LOR(1:PSNM(12))=PSF(sum(PSN(1:11))+1:sum(PSN(1:12)));
    end    
    ...
    for ii=1:NB
        ...
        %Calc. the line profile
        %**********************        
        DATA=Cont_2(IGB(ii,1:NG),IB,DATA,uu);

        if ALGO(2)==1
            %************
            %Fit the data
            %************        
            [~,CHI_T(ii,uu)]=Full_Ins(PARA,DATA,uu);
        elseif ALGO(2)==2
            %*******************************************
            ...
            %Fit the data
            %************
            [~,CHI_T(ii,uu)]=fmincon(FH_INS,[1,0],[],[],[],[],PSB_INS(1:2,1),PSB_INS(1:2,2),[],ALGO_OPT{2});
        end
    end
end
