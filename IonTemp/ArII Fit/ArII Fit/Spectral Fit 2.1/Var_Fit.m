function FIT=Var_Fit(PARA,DATA,FIT,OPT) 
...
for uu=1:NSPF    
    ...
    IGB_CON=FIT.IGB_CON{uu};
    IGB_VAR=FIT.IGB_VAR{uu};
    ...
    for ii=1:NS
        ...
        if PSN(13)<PSNM(13)
            kk=0;
            for jj=1:PSNM(13)
                if PSL(sum(PSNM(1:12))+jj)==0
                    kk=kk+1;
                    IB(jj)=PSF(ii,sum(PSN(1:12))+kk);
                else
                    IB(jj)=PSV(sum(PSNM(1:12))+jj);
                end
            end
        elseif (PSNM(13)>0)
        ...
        %******************************   
        DATA=Cont_2(IGB_CON(ii,1:NG),IB(1),DATA,uu);
        
        DATA=Cont_2(IGB_VAR(ii,1:NG),IB(2),DATA,uu);

        if (ALGO(2)==1)
            %************
            %Fit the data
            %************        
            [VAR_INS(ii,1:2),CHI(ii)]=Full_Ins(PARA,DATA,uu);
        elseif (ALGO(2)==2)
            %*******************************************
            ...
            %Fit the data
            %************
            [VAR_INS(ii,1:2),CHI(ii)]=fmincon(FH_INS,[1,0],[],[],[],[],PSB_INS(1:2,1),PSB_INS(1:2,2),[],ALGO_OPT{2});
        end

        %********************
        %Assign fit variables
        %********************	
        VAR(ii,1:NV)=[VIEW POL B_Z EFP_X EFP_Y EFP_Z kT_DOP X_DOP I_DOP GAM_LOR X_LOR I_LOR IB]; 

        %*******************************
        %Assign instrument fit variables
        %*******************************	
        VAR(ii,NV+1:NV+2)=VAR_INS(ii,1:2);        

        %**************************************
        %Scale and shift the continuous profile
        %**************************************
        NOR=max(DATA.GRID{uu}.IG+IB(1)*IGB_CON(ii,1:NG)+IB(2)*IGB_VAR(ii,1:NG));

        PROFILE{ii}.IGB_CON=IGB_CON(ii,1:NG)*IB(1)/(NOR*VAR_INS(ii,1));
        PROFILE{ii}.IGB_VAR=IGB_VAR(ii,1:NG)*IB(2)/(NOR*VAR_INS(ii,1));
        PROFILE{ii}.IG=DATA.GRID{uu}.IG/(NOR*VAR_INS(ii,1));
        PROFILE{ii}.IGT=DATA.GRID{uu}.IGT/VAR_INS(ii,1);
        PROFILE{ii}.XG=DATA.GRID{uu}.XG-VAR_INS(ii,2);
        PROFILE{ii}.NG=NG;
    end
    ...
    FIT.CHI{uu}=CHI;
    FIT.VAR{uu}=VAR;
    FIT.PROFILE{uu}=PROFILE;
end

%************************************
%Remove temporary FIT sturcture field
%************************************
FIT=rmfield(FIT,'IGB_CON');
FIT=rmfield(FIT,'IGB_VAR');
