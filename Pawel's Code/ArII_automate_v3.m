...
%%%Read in Data%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%Define paths
PATHNAME = '\\mpexserver\ProtoMPEX_Data\McPherson\2017_12_08\';
FILENAME = sprintf('D2Ar_7218_30um_%0.0f.SPE',shot);
%%%Read raw data
[image, header] = readSPE([PATHNAME FILENAME]);
data = im2double(image);
dim_size = size(data);
frame_rate = 1/dim_size(3);
time_vec   = (3.97:frame_rate:4.97);
%%%Get background data 
...
Fiber = 5;
for ii = 1:dim_size(3)
    Signal = data(Fiber,:,ii);
    Signal = Signal./max(Signal);
    S2N = 1.0/abs(min(Signal));
    if S2N > 5
        mid_ind=find(max(data(5,:,ii))==data(5,:,ii));
        Frame=ii;
        break;
    else
        mid_ind = 265;
...
mid_ind = 240;
% mid_ind=find(max(data(5,:,13))==data(5,:,13));
lambda=(lambda0+mid_ind*dpix:-dpix:lambda0-(511-mid_ind)*dpix);
indx=(mid_ind-20:mid_ind+20);
